#!/usr/bin/env python3
"""openssl-rs — Phase 5 courts: `BIGNUM`, differentially.

Each court is a C probe in `courts/phase5/` compiled **twice** — once against the
...
The method is Phase 3's and Phase 4's, for the same reason: a unit test encodes
what the author believes the contract is, whereas a probe measures what the
authority actually does, and the comparison is between two *executions* of the
same program, so the expectation cannot drift. `BN_*` is a surface where that
matters more than usual, because several of its documented behaviours are subtly
different from the analogous C operator (`BN_snprintf`-style surprises, the sign
of a division remainder, `BN_get_word`'s saturation), and a plausible reading of
any of them produces a wrong-but-confident implementation.

What the probe establishes
--------------------------
The observable surface of an opaque type: the value read back through the
conversions, the sign, the bit length, the predicate answers, the return classes,
the error queue after a failure, and the division identity `a == b*q + r` checked
in the probe itself so both sides are held to the same property.

Fault boundaries
----------------
A probe cannot compare a crash. Where the authority faults the probe does not
call, and the candidate's safer behaviour is recorded in
...
(`forensics/tools/phase5_obligations.py`) is where that is recorded; the probe
stays on the implemented surface so that a failure here means a behavioural
divergence rather than a missing symbol.
...
OUT = REPO_ROOT / "artifacts" / "phase5" / "COURTS.json"
GENERATOR = "forensics/tools/phase5_courts.py"
PROBE_DIR = REPO_ROOT / "courts" / "phase5"
PHASE2 = REPO_ROOT / "artifacts" / "phase2"
STAGED = REPO_ROOT / "artifacts" / "phase5" / "probes"
RUN_TIMEOUT_S = "60"

COURTS = [
    ("RT-BN", "rt_bn_probe.c"),
    ("RT-ASN1", "rt_asn1_probe.c"),
    ("RT-ASN1-TEMPLATE", "rt_asn1_template_probe.c"),
...
def main(argv: list[str]) -> int:
    ...
    doc = envelope(kind="phase5-courts", authority=auth.id, inputs=inputs,
