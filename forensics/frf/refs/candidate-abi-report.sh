#!/bin/sh
# openssl-rs FRF CANDIDATE side: report name@version for each symbol named in the
# fixture list, read from the candidate install.s libcrypto.so.3.
#
...
set -eu
LIB=/work/artifacts/phase2/install/lib/libcrypto.so.3
LIST="${1:?usage: candidate-abi-report.sh <symbol-list>}"
TMP=$(mktemp)
