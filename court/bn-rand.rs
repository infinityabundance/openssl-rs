...
//!
//! Every public entry point below reaches
//! [`crate::runtime::rand::RAND_bytes_ex`] / [`crate::runtime::rand::RAND_priv_bytes_ex`],
//! and neither symbol exists in the crate yet: they are Phase 9's and their home is
//! `crypto/rand/rand_lib.c` (the crate's counterpart module, `src/runtime/rand.rs`,
//! has not been written). The transcription is complete and checked against the
//! authority up to that call, and it lands the moment the RAND front does. It is
//! **not** watered down to compile: a version that stubbed the RNG would be a
//! different observable contract, not a smaller one.
//!
//! ## Missing callees, named
//!
//! * `crate::runtime::rand::RAND_bytes_ex` and `RAND_priv_bytes_ex` —
//!   `crypto/rand/rand_lib.c`. The authority's signatures, from
//!   `include/openssl/rand.h`, are
//!   `int RAND_bytes_ex(OSSL_LIB_CTX *libctx, unsigned char *buf, size_t num,
//!   unsigned int strength)` and the `_priv_` twin, and that is the shape the calls
//!   here assume.
//! * `crate::bn::ctx::ossl_bn_get_libctx` — `crypto/bn/bn_ctx.c:243`, an internal
//!   declared in `include/crypto/bn.h:126`. `bnrand` reads its context's library
//!   context through it and hands it to both RAND calls, so it is needed the same
//!   day the RAND front lands. The call below is transcribed as the authority spells
//!   it, `ossl_bn_get_libctx(ctx)`, and that name is not in `src/bn/ctx.rs` today.
//!
//! Every other callee — `BN_zero_ex`, `BN_bin2bn`, `BN_num_bits`, `BN_is_zero`,
//! `BN_is_bit_set`, `BN_cmp`, `BN_ucmp`, `BN_sub`, `BN_set_flags`, `CRYPTO_malloc`
//! and `CRYPTO_clear_free` — exists in the crate today with the authority's
...
//! (Phase 6/7). Their error sites are already staged (`BN_RAND_248`, `_253`, `_271`,
//! `_332`, `_338`, `_385` in `src/runtime/err_sites.rs`), so the later slice lands
//! on arranged ground rather than guessing coordinates.
//!
...
use crate::runtime::rand::{RAND_bytes_ex, RAND_priv_bytes_ex};
...
pub(crate) unsafe fn bnrand(
    ...
    // The authority reads the context's library context here. Internal, from
    // `crypto/bn/bn_ctx.c:243`, and absent from the crate: see the module note.
    ...
    let ret = (|| -> c_int {
        ...
        // SAFETY: `libctx` is the caller's context (null when `ctx` is null, which
        // the RAND front accepts) and `buf` is writable for `buf.len()` bytes per
        // the block above. `RAND_bytes_ex` is Phase 9's and does not exist yet.
...
pub(crate) unsafe fn bnrand_range(
    ...
    if n == 1 {
        ...
        unsafe { BN_zero_ex(r) };
    } else if unsafe { BN_is_bit_set(range, n - 2) } == 0
        && unsafe { BN_is_bit_set(range, n - 3) } == 0
    {
         ...
         */
        loop {
            // SAFETY: `flag` and `strength` are values, `r`/`range`/`ctx` are null
            // or live per this function's `# Safety` section. `n + 1` cannot be
            // reached with `n == c_int::MAX` for a `BIGNUM` this process can hold,
            // and `wrapping_add` keeps a debug build from panicking if it were.
            if unsafe {
                bnrand(
                    flag,
                    r,
...
mod tests {
    ...
    fn a_zero_negative_or_null_range_is_invalid() {
        let r = unsafe { BN_new() };
        // SAFETY: both objects are allocated here and live for the test.
