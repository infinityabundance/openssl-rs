...
| 8.5 | **DH and DHX** | the `DH` object, `DH_*`, `ossl_dh_asn1_meth`, the FFC groups, and the DH provider surfaces. Ninety-seven labels: `src/dh/mod.rs`. **The method table is LANDED (D329):** the twenty-one `DH_meth_*` labels of `crypto/dh/dh_meth.c` are in `src/dh/mod.rs` with `RT-DH` at 66 observations. **The `crypto/ffc/` primitives are LANDED (D330):** the five units D329 named — `ffc_params.c` (seventeen of its eighteen external definitions; `ossl_ffc_params_todata` is withheld on `crypto/param_build_set.c` and `crypto/ffc/ffc_dh.c` and is the sixth row in `forensics/prerequisites.json`'s divergence list), `ffc_params_validate.c`, `ffc_params_generate.c`, `ffc_key_generate.c` and `ffc_key_validate.c` — are `src/ffc/`'s five modules, with `FFC_PARAMS` measured at 96 bytes by `courts/layout/measure-ffc-params.c` and 33 unit tests as the evidence, since the unit has no export for a court arm to cover. The rest lands in the order D329 recorded — the object layer, then `dh_key.c` and `dh_gen.c`. The named-group tables (`crypto/bn/bn_dh.c`'s twenty-six constants and `ffc_dh.c`'s `dh_named_groups[]`) are a separable large data transcription left whole for a follow-up, so `ffc_dh.c` and `ffc_backend.c` have no module. **The object layer, the key layer, the generator and the validators are LANDED (D331):** `crypto/dh/dh_lib.c`, `dh_key.c`, `dh_gen.c`, `dh_check.c` and `dh_depr.c` are `src/dh/object.rs`, `key.rs`, `gen.rs`, `check.rs` and `depr.rs`, so the three `DH_generate_*` names the ledger had deferred are implemented and `RT-DH` grows 66 -> **210 observations**. What is still out: `dh_group_params.c`'s named-group unit (`DH_get_nid`, `DH_new_by_nid`, `ossl_dh_cache_named_group`), `dh_asn1.c`'s `d2i_`/`i2d_DHparams`, the `DH_KDF_X9_42` provider KDF and the `EVP_PKEY_CTX_*dh*` controls. | 8.4 (the shared BN/param idiom) | `RT-DH`, `CT-DH` | 93 |
...
table. The **named-group tables** (`crypto/bn/bn_dh.c`'s twenty-six constants and `ffc_dh.c`'s
`dh_named_groups[]`) are a separable large data transcription whose values only a comparison with
the authority can check, and they are deliberately left whole for a follow-up rather than
half-landed; `DH_set0_pqg`'s named-group cache waits with them. The ledger: phase 8
...
`ossl_param_build_set_*` and `crypto/ffc/ffc_dh.c`'s named-group lookups — so the prerequisite gate's
`unwired_function_in_the_current_stratum` finding for it becomes the sixth row of
`forensics/prerequisites.json`'s divergence list rather than a stubbed body, and `ffc_dh.c` and
`ffc_backend.c` are left with **no module** for the same reason D327 gave `rsa_sp800_56b_check.c`
none. The named-group tables therefore still wait, exactly as the paragraph above says, and so do
