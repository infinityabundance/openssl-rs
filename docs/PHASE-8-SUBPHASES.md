...
| 8.5 | **DH and DHX** | the `DH` object, `DH_*`, `ossl_dh_asn1_meth`, the FFC groups, and the DH provider surfaces. Ninety-seven labels: `src/dh/mod.rs`. **The method table is LANDED (D329):** the twenty-one `DH_meth_*` labels of `crypto/dh/dh_meth.c` are in `src/dh/mod.rs` with `RT-DH` at 66 observations, and the rest lands in the order D329 records — the `crypto/ffc/` primitives, then the object layer, then `dh_key.c` and `dh_gen.c`. The named-group tables are a separable large data transcription left whole for a follow-up. | 8.4 (the shared BN/param idiom) | `RT-DH`, `CT-DH` | 93 |
...
construction vectors are the FFC group arithmetic this slice does not land. **The order the rest
