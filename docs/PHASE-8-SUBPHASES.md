...
`EVP_PKEY_get1_RSA`, `EVP_PKEY_set1_RSA`, `EC_GROUP_new_by_curve_name`,
`EC_GROUP_new_by_curve_name_ex`, `EC_GROUP_get_curve`, `EC_GROUP_method_of`, `EC_POINT_mul`,
`EC_KEY_new_by_curve_name`.
8.7's curve tables landed in D334 and the group object did not, and the two are one landing rather
...
cannot be landed apart, and the two group constructors above are the two labels that show it.
...
8.6's DSA object layer and method table have landed (D333), so what
is open there is the DER-dependent tail: `DSA_sign`, `DSA_verify`, `DSA_size`, `DSA_print`,
`DSA_print_fp`, `DSAparams_dup`, `DSAparams_print`, `DSAparams_print_fp`, `d2i_DSA_SIG`,
`i2d_DSA_SIG`, `d2i_DSAparams` and `i2d_DSAparams`, of which the first three are withheld on the
`i2d_DSA_SIG`/`d2i_DSA_SIG` codec that `crypto/asn1_dsa.c` and `crypto/packet.c` hold and the rest
on 8.8's ASN.1 method machinery.
