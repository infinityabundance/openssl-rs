...
| 8.3 | **The AEAD and mode primitives** | GCM, CCM, XTS, key wrap, Poly1305, ChaCha20-Poly1305, and the remaining `CRYPTO_*` mode functions. Fifty labels, all of them `src/modes/mod.rs`'s. **The low-level half is LANDED (D224–D229):** all fifty `modes.h` exports are in — key wrap (D224), GCM (D225), CCM (D226), XTS (D227) and OCB (D229) — and their evidence is the `RT-CIPHER`/`CT-CIPHER` families this row names. Poly1305 and ChaCha20-Poly1305 are **not** this stratum's exports (D228): they are internal units (`crypto/poly1305/poly1305.c`, `crypto/chacha/chacha_enc.c`) and provider rows, with no `libcrypto` symbol. **The provider half has begun (D230–D233), and its first AEAD engine of its own is in (D238):** `deflt_ciphers[]` now publishes the twelve AES key-wrap rows, the six CBC-CTS rows (AES-128/192/256 and Camellia-128/192/256, D231), the two AES-XTS rows (D232), the three AES-OCB rows (D233) and the three AES-CCM rows (D238, with `ciphercommon_ccm.c`'s engine transcribed whole and its failure arms raising the authority's own provider reasons), leaving the SIV rows and ChaCha20 open; the AES-GCM rows are **deferred to Phase 9** because their no-IV encrypting arm and their TLS arm both call RAND_bytes_ex (D234), and since D240 the deferral carries a **second** blocker: `ossl_cipher_generic_initkey` also stores `PROV_LIBCTX_OF(provctx)` on `ctx->libctx`, which this crate's NULL `provctx` cannot supply, so `RAND_bytes_ex` alone would answer with the global context where the authority answers with the fetching `OSSL_LIB_CTX`. That second blocker is measured, not asserted: `forensics/atlas/provider-algorithms.json`'s `provider_context` block reads the crate and the authority's template and fails on any of the three facts moving. **The SIV rows are open with a named in-stratum prerequisite (D239):** `crypto/modes/siv128.c` is transcribed whole (`src/modes/siv128.rs`, with the memory-safety divergence D239 records), but the rows drive it through `EVP_MAC_fetch(…, "CMAC", …)`, and the crate publishes no `OSSL_OP_MAC` row yet — the measurement is that `EVP_MAC_fetch(NULL, "CMAC", NULL)` answers 1 on the authority and 0 on the candidate, so the CMAC provider row (`providers/implementations/macs/cmac_prov.c`, one of Phase 8's nine open `OSSL_OP_MAC` rows) is what the six SIV rows wait on. | 8.2 | `RT-CIPHER`, `CT-CIPHER` | 50 |
...
so what remains open for 8.3 is most of the default provider's `deflt_ciphers[]` — the twelve AES
key-wrap rows landed in D230, the six CBC-CTS rows in D231, the two AES-XTS rows in D232, the
three AES-OCB rows in D233 and the three AES-CCM rows in D238, leaving the SIV rows and ChaCha20
open; the AES-GCM rows are
