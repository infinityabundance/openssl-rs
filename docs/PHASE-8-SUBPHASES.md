...
the ledger moves to implemented 656, deferred 1, open 129 over the same 786 owned. What the 34
remaining `src/ec/mod.rs` names wait on is **not** one stratum, and the split is recorded here
...
`EVP_PKEY_assign()`, which is Phase 11's by the same coordinate; one -- `ECDH_KDF_X9_62`, whose
whole body is `EVP_KDF_fetch()` of the X9.63 KDF -- is Phase 9's provider KDF; and **six are
neither**, which is where this paragraph corrects the brief's own reading: the four `EC_POINT_*`
hex/BN codecs of `ec_print.c`/`ec_deprecated.c` and `ec_asn1.c`'s `ECDSA_SIG_get0_r` and
`ECDSA_SIG_get0_s` reach only landed names (`EC_POINT_point2buf()`, `EC_POINT_oct2point()`,
`BN_bin2bn()`, `BN_bn2binpad()`, `ossl_to_hex()`) and are same-stratum work a later EC slice can
land.
...
DSA_size, d2i_DSA_SIG and i2d_DSA_SIG and what is open there is `DSA_print`, `DSA_print_fp`,
`DSAparams_dup`, `DSAparams_print`, `DSAparams_print_fp`, `d2i_DSAparams` and `i2d_DSAparams`,
all withheld on 8.8's ASN.1 method machinery.
