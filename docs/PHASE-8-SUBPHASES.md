...
MAC operation is six of its nine `deflt_macs[]` rows landed (CMAC, HMAC, SIPHASH, POLY1305 and the
two BLAKE2 rows) with `KMAC-128`/`KMAC-256` open and GMAC a measured Phase 9 hand-off on the
`AES-*-GCM` cipher rows whose modes it will accept, leaving ChaCha20 as the last open cipher of the
subphase; the AES-GCM rows are
