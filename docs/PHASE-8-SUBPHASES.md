...
| 8.9 | **The `pem.h` helpers Phase 8 owns** and the `*_asn1_meth` bodies that unblock Phase 7's remaining Phase-8 deferrals | the thirty `pem.h` names — the `PEM_read_*`/`PEM_write_*` family for `DHparams`, `RSA`, `DSA` and `EC` keys. **The twenty-four landable rows are LANDED (D350), the six private-key readers withheld:** `crypto/pem/pem_all.c`'s `IMPLEMENT_PEM_*` expansions are written out in `src/pem/key_legacy.rs` — 14 readers and 16 writers — because the crate has no C preprocessor, and what is withheld is `PEM_read[_bio]_{RSA,DSA,EC}PrivateKey`, each `PEM_read[_bio]_PrivateKey` over a `pkey_get_*` helper that calls `EVP_PKEY_get1_{RSA,DSA,EC_KEY}` (Phase 8's own open rows, waiting on `standard_methods[]` and D341's cycle) **and** the `PEM_read[_bio]_PrivateKey` pair itself (`crypto/pem/pem_pkey.c`, Phase 7's, handed to Phase 10/13): two independent absences, named rather than stubbed. `RT-PEM-KEY` is registered and passing at **102 observations**, zero residual, and the ledger moves implemented **721 → 745**, open **65 → 41**. The ten `*_PUBKEY`/`X509_*` `pem_all.c` rows are Phase 11's and not this row's, and the `*_asn1_meth` bodies the row's title also names are unchanged — the method objects still cannot exist before Phase 11's `X509_PUBKEY`/`PKCS8_PRIV_KEY_INFO` (D341). **D363 opens the decoder chain these six wait on:** `crypto/encode_decode/decoder_meth.c`'s object, dispatch scan, accessors and parameter pass-throughs have landed as their own module, **D364 adds `crypto/encode_decode/decoder_lib.c`** with `decoder_meth.c`'s context trio, and **D365 adds `decoder_pkey.c`'s cache** (filling slots 11 and 20) with the four `OSSL_DECODER_CTX_set_passphrase*` setters. What remains between this row's readers and the ledger is `decoder_pkey.c`'s pkey half, `OSSL_DECODER_CTX_add_extra`/`decoder_process`/`from_bio` and `crypto/pem/pem_pkey.c`, so the six readers stay open and the count this row reports does not move. Thirty labels: `src/pem/key_legacy.rs` | 8.8 | `RT-PEM-KEY` | 30 |
