...
* **Phase 8 owns glue that lives in other directories, and Phase 7 says so.** All
  ...
  `dsa.h` and `ec.h` and their bodies are `crypto/rsa/rsa_pmeth.c`, `crypto/dh/dh_pmeth.c`,
    `crypto/dsa/dsa_pmeth.c` and `crypto/ec/ec_pmeth.c`'s. Ownership
...
and its twelve objects are `crypto/rsa/rsa_ameth.c`,
`crypto/dh/dh_ameth.c`, `crypto/dsa/dsa_ameth.c` and `crypto/ec/ec_ameth.c`'s. `ownership_audit.py`
...
`forensics/prerequisites.json` carried seven rows whose `owner_phase` is 8 at the bootstrap:
...
objects 8.8 builds. **None was discharged at the bootstrap**, and the gate reported them as
`planned_prerequisites` rather than as gaps, which is the census it publishes for exactly
this state. Every one has since been discharged and its row removed -- the last two,
`evp_cleanup_int` and `evp_app_cleanup_int`, with the 8.8 `EVP_PKEY_METHOD` slice that made
the seventh call of the former reachable.
...
D346 lands three more: `DES_random_key`, `DH_KDF_X9_42` and `ECDH_KDF_X9_62`, the last two over the
KDF rows the same commit adds to the default provider.
D347 lands 8.7's DER and printer slice: `ECPARAMETERS_new`, `ECPARAMETERS_free`, `ECPARAMETERS_it`,
`ECPKPARAMETERS_new`, `ECPKPARAMETERS_free`, `ECPKPARAMETERS_it`, `d2i_ECPKParameters`,
`i2d_ECPKParameters`, `EC_GROUP_get_ecparameters`, `EC_GROUP_get_ecpkparameters`,
`EC_GROUP_new_from_ecparameters`, `EC_GROUP_new_from_ecpkparameters`, `d2i_ECParameters`,
`i2d_ECParameters`, `d2i_ECPrivateKey`, `i2d_ECPrivateKey`, `o2i_ECPublicKey`, `i2o_ECPublicKey`,
`ECPKParameters_print`, `ECPKParameters_print_fp`, `ECParameters_print` and `ECParameters_print_fp`.
D362 lands the eight 8.4/8.6/8.7 printers: `RSA_print`, `RSA_print_fp`, `DSA_print`, `DSA_print_fp`,
`DSAparams_print`, `DSAparams_print_fp`, `EC_KEY_print` and `EC_KEY_print_fp`. They are
crypto/rsa/rsa_prn.c, crypto/dsa/dsa_prn.c and crypto/ec/eck_prn.c's, wrapped around
crypto/evp/p_lib.c's internal print_pkey, whose encoder-first arm now answers a zero-encoder context
on every key this crate can build -- so the authority's own fall-through to the method object's
priv_print/params_print/pub_print callbacks is the live path rather than a call-site reduction.
D369 completes the unit D349 left partial and closes the six names the clause below used to carry:
crypto/x509/x_pubkey.c's object layer, its X509_PUBKEY_INTERNAL item and x509_pubkey_decode land
in `src/x509/x_pubkey.rs`, crypto/pem/pem_pkey.c's read half lands as `src/pem/pem_pkey.rs`,
and the six hand-written readers `PEM_read_RSAPrivateKey`, `PEM_read_bio_RSAPrivateKey`,
`PEM_read_DSAPrivateKey`, `PEM_read_bio_DSAPrivateKey`, `PEM_read_ECPrivateKey` and
`PEM_read_bio_ECPrivateKey` land in `src/pem/key_legacy.rs` over the three `pkey_get_*` helpers
beside them. The ledger moves implemented **780 -> 786** and open **6 -> 0** over the same 786
owned, so this stratum's working set is whole, and `RT-PUBKEY` is registered and passing at
**100** observations with zero residuals. Working set whole is not the stratum sealed: the four
`crypto/ec/ecx_meth.c` rows are still withheld from both `standard_methods[]` tables
(`D-PKEY-AMETH-3`), the 137 provider registration rows above are still neither implemented nor
handed on, and the twenty `x509.h` and eight `pem.h` exports this slice lands are IMPLEMENTED and
nothing more. **D372 closes the first of those three clauses**: the four `crypto/ec/ecx_meth.c` rows
are no longer withheld from either `standard_methods[]` table (`D-PKEY-AMETH-3` is superseded), so
the 137 provider registration rows and the twenty-eight `x509.h`/`pem.h` exports are what remain
outside this stratum's working set.

D353 lands 8.8's slice: the eleven in-reach `standard_methods[]` rows over `src/rsa/ameth.rs`,
`src/dh/ameth.rs`, `src/dsa/ameth.rs` and `src/ec/ameth.rs`; `EVP_PKEY_type`; the `EvpPkey` legacy
block; `crypto/evp/p_legacy.c` as `src/evp/p_legacy_assign.rs`; and the twelve legacy accessors
with the four other `crypto/evp` exports they close over -- `EVP_PKEY_assign`,
`EVP_PKEY_encrypt_old`, `EVP_PKEY_decrypt_old`, `EVP_PKEY_get0_hmac`, `EVP_PKEY_get0_poly1305`,
`EVP_PKEY_get0_siphash`, `EVP_PKEY_get_ec_point_conv_form`, `EVP_PKEY_get_field_type`. Twenty-one
exports land and `RT-AMETH` is registered and passing at 275 observations with zero residuals. The
eight `RSA_print`/`DSA_print`/`EC_KEY_print` family printers stay open, because their callee
`EVP_PKEY_print_private` is absent from the crate's compiled surface.

D354 lands the last three same-stratum `d2i` names 8.8 owed and closes that family:
`crypto/asn1/d2i_param.c` is `src/asn1/d2i_param.rs` and `crypto/asn1/d2i_pu.c` is
`src/asn1/d2i_pu.rs`, each its own module because each is its own authority unit, so
`d2i_KeyParams`, `d2i_KeyParams_bio` and `d2i_PublicKey` are built. Both units close over the
eleven-row `standard_methods[]` table and the four `*_ameth.c` objects D353 landed -- the first
reads `ret->ameth->param_decode` after `EVP_PKEY_set_type`, and the second switches on
`EVP_PKEY_get_base_id` into `d2i_RSAPublicKey`, `d2i_DSAPublicKey` and `o2i_ECPublicKey` -- which
is why neither could land before that slice. Three exports land, the ledger moves implemented
**766 -> 769** and open **20 -> 17** over the same **786** owned, and `RT-AMETH` grows
275 -> **291 observations** with zero residuals. The three `EVP_PKEY_meth_*` names,
crypto/evp/pmeth_lib.c's `EVP_PKEY_METHOD` table, the eight printers and the six `crypto/pem/`
private-key readers are unchanged.

D355 lands the rest of 8.8's registry: the **second** `standard_methods[]` of `crypto/evp/pmeth_lib.c`
-- an array of `pmeth_fn` **accessors** rather than of objects -- carries its six in-reach rows in
ascending `pkey_id` order (6, 28, 116, 408, 912, 920) over the four whole `*_pmeth.c` units
`src/rsa/pmeth.rs`, `src/dh/pmeth.rs`, `src/dsa/pmeth.rs` and `src/ec/pmeth.rs`; `EvpPkeyCtx` gains
the `pmeth`, `engine` and `flag_call_digest_custom` members the callbacks read (`pkey_ctx_is_pss` is
nothing but `ctx->pmeth->pkey_id`, and `pkey_dh_keygen`/`pkey_rsa_keygen` assign with it), with the
legacy block's internal layout measured by `courts/layout/measure-evp-pkey-ctx.c`; and
`EVP_PKEY_meth_find`, `EVP_PKEY_meth_get0` and `EVP_PKEY_meth_get_count` land over it. Three exports
land, the ledger moves implemented **769 -> 772** and open **17 -> 14** over the same 786 owned, and
`RT-AMETH` grows 291 -> **420 observations** with zero residuals. The four `crypto/ec/ecx_meth.c` rows
are withheld from **both** tables under `D-PKEY-AMETH-3` — **D372 appends them to both**, so this
clause is closed — and
`forensics/prerequisites.json`'s `ossl_rsa_pkey_method` deferral retires while its
`evp_app_cleanup_int` neighbour becomes a plain ownership row in the same slice; both rows
are since discharged and removed, the latter when `evp_cleanup_int` — the only caller of
`evp_app_cleanup_int` — was built and wired into `OPENSSL_cleanup`.

**Open exports (checked against the ledger):** none. D369's landing closed the last six, and they
are named in the landed clause above. The two crypto/pem/pem_pkey.c names that stay unbuilt --
PEM_read_bio_Parameters and its _ex spelling -- are **withheld**, not open: their only successful
arm on this revision is the provider decoder this crate does not publish (D-DECODER-ABSENT-1,
...
the six names this paragraph had called landable**, and **D347 lands the twenty-two this split had
called 8.8's**: the `ec_asn1.c` template machinery and DER entry points, the four `EC_GROUP_*`
builders, `eck_prn.c`'s printers and the parameter printer `ec_ameth.c` owns are `src/ec/asn1.rs` and
`src/ec/prn.rs` whole, and none of them reaches a Phase 11 name, so what is left of `src/ec/mod.rs`
is **two** names -- EC_KEY_print and EC_KEY_print_fp, the eck_prn.c pair whose body reaches
the Phase 10 encoder context, and **D362 lands both of them**;
**D353 lands the three `EVP_PKEY_*_EC_KEY` legacy accessors this
paragraph had called Phase 11's** over the legacy block of the crate's EVP_PKEY object, together
with `crypto/ec/ec_ameth.c`'s method object and the eleven-row table they are found through. The twenty-eighth was the X9.63 wrapper this paragraph had called Phase 9's
provider KDF; **D346 measured the census and landed it** -- provider-algorithm-plans.json gives the
`default/OSSL_OP_KDF` operation to phase 8 and the row it fetches is therefore this stratum's.
The six that left
...
plan's
"8.8's machinery" grouping and D341's own measurement disagreed — so what was open there were the four
dsa_prn.c printers, DSA_print, DSA_print_fp, DSAparams_print and DSAparams_print_fp,
withheld on the Phase 10 encoder context, and **D362 lands all four**;
**D353 lands the three `EVP_PKEY_*_DSA` legacy accessors
this paragraph had named**, together with `crypto/dsa/dsa_ameth.c`'s four method rows.
...
only the `RSA_PSS_PARAMS`/`RSA_OAEP_PARAMS` templates of the same unit remained withheld, on
`X509_ALGOR` and therefore Phase 11. **D348 lands that last half**, because it first lands
`crypto/asn1/x_algor.c` whole as `src/asn1/x_algor.rs`: with `X509_ALGOR_it` and `X509_ALGOR_free` in
the crate, the two templates, their two free hooks and the seven/six `IMPLEMENT_ASN1_FUNCTIONS`
exports over them join `src/rsa/asn1.rs`, the eleven names leave the Phase 8 ledger's `open` list, and
`rsa_asn1.c` becomes whole.
...
4. **The default provider's algorithm table is `providers/defltprov.c`'s, and the four
   `*_prov` files this stratum needs are generated.** `digestcommon.c.in`, `sha3_prov.c.in`
   and
   `blake2_prov.c.in` are `.c.in` templates the build expands into
