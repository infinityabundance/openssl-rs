...
is **five** names -- `EC_KEY_print`, `EC_KEY_print_fp`, `EVP_PKEY_get0_EC_KEY`,
`EVP_PKEY_get1_EC_KEY` and `EVP_PKEY_set1_EC_KEY`. The first two are the ASN.1 method object
D341 shows needs Phase 11's `X509_PUBKEY()`/`PKCS8_PRIV_KEY_INFO()` bodies (its algorithm-identifier
need is met by D348); the
three are the legacy accessors and wait on `evp_pkey_get_legacy()`/`EVP_PKEY_assign()`, Phase 11's by
the same coordinate. The twenty-eighth was the X9.63 wrapper this paragraph had called Phase 9's
