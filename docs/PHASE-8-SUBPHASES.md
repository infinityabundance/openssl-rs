...
**Open exports (checked against the ledger):** `RSA_print`, `RSA_print_fp`, `DSA_print`,
`DSAparams_print`, `EC_KEY_print`, `PEM_read_bio_RSAPrivateKey`.
...
the Phase 10 encoder context. **D353 lands the three `EVP_PKEY_*_EC_KEY` legacy accessors this
...
withheld on the Phase 10 encoder context; **D353 lands the three `EVP_PKEY_*_DSA` legacy accessors
