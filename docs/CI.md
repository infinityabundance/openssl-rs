...
same as for a merge: run `forensics/tools/implemented_surface.py`, the three
`phase*_obligations.py`, `ownership_audit.py`, `prototype_court.py`, `phase_state.py` and
`render_status.py` (or just `evidence_determinism.py`, which runs them in dependency
order), then commit what they wrote. What a staging commit may legitimately leave stale
...
`ownership-audit.json`, `prototype-court.json`, the three `phase*-obligations.json`,
`phase-state.json`, `phase-state.md` and `STATUS.md`. If a committed artefact is stale
relative to its generator, every claim built on it is unverifiable, so staleness is a
hard failure rather than a warning. The list is authoritative in the tool's `COMPARED`
table, not here; this paragraph said "the two obligation ledgers" until D79.
