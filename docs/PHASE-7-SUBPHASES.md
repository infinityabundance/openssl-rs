...
| 7.1 | **The fetch core** | `crypto/core_algorithm.c` whole (`ossl_algorithm_do_all`, `ossl_algorithm_get1_first_name`); `crypto/core_fetch.c` whole (`ossl_method_construct`); the `OSSL_METHOD_STORE` object, its lock, its caches and its `opbits` from `crypto/evp/evp_fetch.c` (`ossl_method_store_new`, `_free`, `_cache_flush`, `_remove_all_provided`, `_add`, `_fetch`, `_cache_set`, `_do_all`, `_num`) | 6 | `RT-FETCH` | a third-party provider's algorithm is fetched through `ossl_method_construct`, cached, and found again from the cache; the two deferrals D132/D134 recorded are discharged and their rows removed from `forensics/prerequisites.json` |
