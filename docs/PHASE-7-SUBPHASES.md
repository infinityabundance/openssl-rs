...
By declaring header, over the 950 owned rows, the stratum is: `evp.h` 832, `pem.h` 34,
`kdf.h` 40, `hpke.h` 20, `hmac.h` 12, `cmac.h` 9 and `asn1.h` 3 — the generated
`forensics/phase7-obligations.json`'s `owned_by_header`, which is where these are read
from. By library: `libcrypto` for all of it, `libssl` 0 — Phase 7 adds nothing to
`libssl`.
