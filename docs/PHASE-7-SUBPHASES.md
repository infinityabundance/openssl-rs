...
`evp_cleanup_int` is owed to **7.4** (`EVP_PBE_cleanup` is `evp_pbe.c`'s) and `EVP_add_alg_module` to **7.4** as well (`X509V3_get_value_bool` is Phase 11's); both are recorded in `forensics/prerequisites.json` rather than stubbed.
