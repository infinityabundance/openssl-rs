...
| 7.1 | **The fetch core** | `crypto/core_algorithm.c` whole (`ossl_algorithm_do_all`, `ossl_algorithm_get1_first_name`); `crypto/core_fetch.c` whole (`ossl_method_construct`); the `OSSL_METHOD_STORE` object, its lock, its caches and its `opbits` from `crypto/evp/evp_fetch.c` (`ossl_method_store_new`, `_free`, `_cache_flush`, `_remove_all_provided`, `_add`, `_fetch`, `_cache_set`, `_do_all`, `_num`) | 6 | `RT-FETCH` | **PARTLY LANDED (D139).** `crypto/core_algorithm.c` is transcribed whole in `src/evp/algorithm.rs`: `ossl_algorithm_do_all`, `algorithm_do_this` and `algorithm_do_map`, with the three return conventions kept apart because two of them read like errors and are not, and with `OSSL_ALGORITHM` completed in `src/provider/activate.rs` where Phase 6 declared it. **D132's deferral is discharged and its row removed from `forensics/prerequisites.json`**, so the plan-reconciliation census for Phase 7 drops by two and the gate's blocking list goes 15 to 14 — a decrease, which is why nothing had to record it. Still open: `ossl_method_construct` and the method store, which is where the constructor the walk calls is written. The court lands with the rest of 7.1 |
