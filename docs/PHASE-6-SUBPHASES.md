...
| 6.7b | the grammars | `crypto/property/property_parse.c` (763), `property_query.c` (80), and the cache's `get`/`set` | 6.7a | the fetch at 6.8 | **IN PROGRESS** (D112). Landed so far: `crypto/bsearch.c` mirrored as `src/runtime/bsearch.rs`; the authority's character-class table **generated** from `crypto/ctype.c` by `gen_ctype_table.py`, with `src/runtime/ctype.rs` gaining the five classes the grammar needs and a test that checks its five hand-reasoned range tests against that table for every byte value; the list and definition types as `src/property/list.rs`; and `property_query.c` as `src/property/query.rs`. `list.rs` and `query.rs` are declared now that the grammar calls them. Landed: the whole of `property_parse.c`'s grammar — the leaf parsers for names, decimal/hex/octal numbers, quoted and unquoted values, the dispatch on the first character, `stack_to_property_list` with its sort and its duplicate-name refusal, `ossl_parse_property`, `ossl_parse_query`, `ossl_property_match_count` and `ossl_property_merge` — with seven unit tests over the refusal cases and the merge join. **Still owed:** `ossl_property_list_to_string` and its `put_char`/`put_str`/`put_num` helpers, and `defn_cache.c`'s `ossl_prop_defn_get`/`set`. Every symbol in the module carries an `allow(dead_code)` naming 6.8, because the whole grammar is the interface the provider fetch is written against and nothing calls it yet |
