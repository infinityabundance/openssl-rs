...
| 6.7b | the grammars | `crypto/property/property_parse.c` (763), `property_query.c` (80), and the cache's `get`/`set` | 6.7a | the fetch at 6.8 | **IN PROGRESS** (D112). Landed so far: `crypto/bsearch.c` mirrored as `src/runtime/bsearch.rs`; the authority's character-class table **generated** from `crypto/ctype.c` by `gen_ctype_table.py`, with `src/runtime/ctype.rs` gaining the five classes the grammar needs and a test that checks its five hand-reasoned range tests against that table for every byte value; the list and definition types as `src/property/list.rs`; and `property_query.c` as `src/property/query.rs`. `list.rs` and `query.rs` are committed but **not yet in the module tree**, because their only caller is the grammar and the grammar is the next step — declaring them now would mean nine `allow(dead_code)` markers on interfaces rather than on obligations. Still owed: `property_parse.c`'s leaf parsers and the two entry points, `ossl_property_match_count`, `ossl_property_merge`, `ossl_property_list_to_string`, and `defn_cache.c`'s `ossl_prop_defn_get`/`set`. Its evidence remains the provider fetch and 6.12's third-party provider: the engine has no export |
