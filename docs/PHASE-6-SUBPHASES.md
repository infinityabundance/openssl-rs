...
| 6.1 | Prototype gap closure (item 4 of the review) | nothing — evidence | — | `ABI-PROTOTYPE`, generated declaration plane | **COMPLETE**: the 162 `declaration_is_generated` exports are type-checked by a generated compile-time function-pointer assertion, so a macro-generated signature is verified *after* expansion rather than reported as unjudgeable |
| 6.2 | Core-runtime addendum | the 25 Phase 3 exports 6.0 recorded `open` (the `OSSL_ERR_STATE_*` saver, the `OSSL_trace_*` surface under this profile's `OPENSSL_NO_TRACE`, `OSSL_sleep`, the thread-count trio, `OPENSSL_atexit`/`die`/`isservice`/`issetugid`/`thread_stop*`, `err_free_strings_int`) | 6.0 | `RT-RUNTIME-EXT` | the reopened Phase 3 ledger is at zero open and the stratum re-closes with its seal amended by an appended correction section |
