...
| 6.8b | `add_builtin`, the core dispatch table, and the predefined table | `provider_core.c` §§950–1,160 and 2,300–2,679: `ossl_provider_info_add_to_store`'s builtin path, the whole `core_*` dispatch (`core_gettable_params`, `core_get_params`, `core_get_libctx`, `core_thread_start`, `core_new_error`, `core_get_libctx`, the `OSSL_CORE_HANDLE` surface), `provider_predefined.c`'s table, and `OSSL_PROVIDER_add_builtin` | 6.8a | `RT-PROVIDER` | a builtin provider can be *registered* without a shared object, which is the only way to exercise the registry before a provider module exists. The core dispatch table is observed as the authority observes it: through a provider registered this way, since `OSSL_PROVIDER_get0_dispatch` answers the table the provider published |
