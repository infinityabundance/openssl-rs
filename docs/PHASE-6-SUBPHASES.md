...
| 6.5 | `OSSL_PARAM` | `params.c` 61 and `param_build.c` 20 — the descriptor substrate every provider call is made of | 6.3, 6.4 | `RT-PARAM` | **IN PROGRESS** (D103): all 81 exports implemented across `src/params/{mod,dup,from_text,build}.rs`, and every one is checked by **both** prototype planes (`checked=1023 mismatches=0 unclassified=0 unreadable=0`, type plane `checked=1039 mismatches=0 unmapped=0`) after two instrument fixes this subphase found. The differential court is the next commit: by D73's rule nothing here is closed until `RT-PARAM` observes it, so the ledger moving 161 open → 80 open (81 implemented) is arithmetic, not an exit criterion met |
