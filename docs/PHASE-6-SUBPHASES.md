...
| 6.6d | the child context | `OSSL_LIB_CTX_new_child` and the `ischild` flag, which needs `ossl_provider_init_as_child` | 6.6c, 6.8 | `RT-LIBCTX` | **COMPLETE (D131).** `OSSL_LIB_CTX_new_child` -- the last open export of this stratum and of the whole Phase 6 ownership set -- is written in `src/context/mod.rs`, and `OSSL_LIB_CTX_free` gains the `ischild` arm that calls `ossl_provider_deinit_child` before the slot objects are released. `ischild` is set **last**: a failure anywhere above leaves a context the free tears down as an ordinary one, rather than one whose teardown calls `ossl_provider_deinit_child` on globals whose upcalls are NULL. `RT-LIBCTX` gains 14 observations for it, four of which are the validation-precedes-registration contract that 6.8e's `ossl_provider_init_as_child` implements |
...
| 6.6f | the algorithm dispatch walk | `crypto/core_algorithm.c` | 6.6b, 6.8 | `RT-LIBCTX` | `ossl_algorithm_do_all` — the walk over a provider's algorithms that the legacy method enumerations are built on  **NOT BUILT and reassigned to Phase 7 (D132).** `ossl_algorithm_do_all`'s only authority caller is `core_fetch.c`'s `ossl_method_construct`, the fetch path, and nothing in this crate references it, so no court could reach it and no ledger sees it. It is recorded as a Phase 7 prerequisite in `forensics/prerequisites.json`  |
| 6.6g | `OSSL_LIB_CTX_load_config` | the tenth export | 6.10 | `RT-LIBCTX` | **COMPLETE (D129).** The one-line forward it was always going to be, and its *contract* is three details the line does not show: it passes an explicit **zero** flag word rather than `DEFAULT_CONF_MFLAGS`, so a missing file and a failing module are errors here where the automatic loader tolerates them; it answers `> 0` while the inner call can answer **-1**, so it is a boolean rather than a pass-through; and `config_diagnostics` in the file is set on the **context it was loaded into**. `RT-LIBCTX` gains 19 observations for it (91 -> 110), including the pair that pins the `> 0` -- the same file through `CONF_modules_load_file` answering -1 and through this function answering 0. The probe deliberately does **not** observe the NULL-filename-with-`OPENSSL_CONF`-unset case, which reaches `CONF_get1_default_config_file`'s recorded divergence |
...
| 6.10 | CONF module registry | the 18 hand-offs from Phase 4 and Phase 5 — `crypto/conf/conf_mod.c` | 6.8, 6.9 | `RT-CONF-MOD` | module activation through configuration, `CONF_modules_load*`, the imodule/module accessors, and the diagnostics flag interaction D50 recorded **SPLIT into 6.10a-6.10d, and the split is measured rather than guessed (D121).** A symbol sweep over the four translation units this subphase is made of -- `conf_mod.c` (764), `conf_api.c` (214), `conf_sap.c` (82), `conf_mall.c` (38) -- against everything the crate defines finds 58 names that look missing, of which fourteen are the untyped `OPENSSL_sk_*` the crate already has and two are renames (`OPENSSL_malloc` is `CRYPTO_malloc`). What is genuinely absent falls into three groups, and the first is why the documented order was wrong: **6.10a is three units, because D121 missed two of them (D122):** `crypto/sparse_array.c` (216 lines, and the crate has none), then `crypto/threads_common.c`'s per-context thread-local family (414 lines) -- which is what `CRYPTO_THREAD_get_local_ex` turns out to be, and which RCU, `err.c`, `rand_lib.c`, `rsa_ossl.c` and `async.c` all share -- and only then the RCU layer. Following the call rather than the include is what found them; from `crypto/threads_pthread.c` alone the chain is invisible. **The RCU layer**, because `CONF_modules_load` creates `module_list_lock` through `ossl_rcu_lock_new(1, NULL)` and every `CONF_modules_*` entry point takes it. RCU's implementation for this profile is in `crypto/threads_pthread.c` -- there is no `crypto/rcu.c` -- and it is `struct rcu_lock_st` (`cb_items`, `ctx`, `qp_group`, `id_ctr`, `group_count`, `reader_idx`, `next_to_retire`, `current_alloc_idx`, `writers_alloced`, three `pthread_mutex_t`s and two `pthread_cond_t`s), `struct rcu_qp { uint64_t users; }`, `struct thread_qp { qp, depth, lock }`, `struct rcu_thr_data { thread_qp[10] }` with `MAX_QPS 10`, and eleven functions. The read path is the interesting half and the reason 6.6e-ii had to precede it: it keeps per-thread state under `CRYPTO_THREAD_LOCAL_RCU_KEY` **in the lock's own context** and registers `ossl_rcu_free_local_data` as a thread-exit handler through `ossl_init_thread_start`. `get_hold_current_qp` is a re-try loop over `reader_idx` with an acquire-increment of the current quiescent point's `users` and a relaxed decrement when the index moved; `ossl_synchronize_rcu` takes the write lock, swaps `cb_items` aside, allocates the next quiescent point, waits for in-order retirement on `prior_signal`, then waits for the reader count to reach zero before invoking the callbacks. The primitives it needs **already exist**: `src/runtime/thread.rs` carries `ossl_crypto_condvar_*` over `pthread_mutex_t`/`pthread_cond_t`, which is what the authority's own RCU uses, so 6.10a is a transcription rather than a new synchronisation design. **6.10b is the registry**: `conf_mod.c`'s fifteen exports plus `ossl_config_modules_free`, over `supported_modules` and `initialized_modules` (two `STACK_OF` pointers swapped under the RCU write lock, which is why 6.10a is separate). **6.10c is `conf_sap.c` and `conf_mall.c`**: `OPENSSL_config`/`OPENSSL_no_config`/`ossl_config_int`/`ossl_no_config_int` and the default-method initialiser, which is where the `OPENSSL_load_builtin_modules` fan-out lives. **6.10d is `ASN1_add_oid_module`**, the one of the six builtin modules this stratum owns. The fan-out itself is a recorded residual of the same shape as 6.8c's: of the six modules `OPENSSL_load_builtin_modules` registers, `ENGINE_add_conf_module` and `ENGINE_load_builtin_engines` are ENGINE's (**enabled in this profile** -- `configdata.pm` records `engine` and `"engine" => "1"`), `ossl_provider_add_conf_module` is 6.8d's, `ossl_random_add_conf_module` is Phase 9's, the deprecated `EVP_add_alg_module` is Phase 7's, and `ossl_config_add_ssl_module` is libssl's. A config file naming `openssl_init` therefore observes the difference, and the registry registers the modules that exist while naming the ones that do not. The court is `RT-CONF-MOD`. **6.10b, 6.10c and 6.10d are COMPLETE (D127).** `src/runtime/confmod/mod.rs` carries the registry's fifteen exports and `ossl_config_modules_free`, `src/runtime/conf/sap.rs` carries `ossl_config_int` and `ossl_no_config_int`, `src/runtime/init.rs` carries the config step and the re-entrancy thread-local, and `src/runtime/confmod/asn1.rs` carries `ASN1_add_oid_module`. `RT-CONF-MOD` observes 161 behaviours with zero residuals, including the four error coordinates the raise-site generator produces and the left-justified `retcode=%-8d` field, and it is the court that reaches the RCU layer: `conf_mod.c` is RCU's only consumer in this build, which narrows `D-RCU-4`'s consequence from "no differential court can be written for RCU" to "one is written and this is it". Six of the seven modules `OPENSSL_load_builtin_modules` registers are still absent and remain the recorded fan-out residual named above -- the probe names `oid_section` and nothing else, on purpose. **6.10e is COMPLETE too (D128).** `ossl_config_add_ssl_module`, `ssl_module_init` and `ssl_module_free` are in `src/runtime/conf/conf_ssl.rs`, so `ssl_conf` is the second of the seven `OPENSSL_load_builtin_modules` registrations this crate makes and `conf_ssl.c`'s store is filled by a configuration file rather than only by a test. The paragraph above called that symbol libssl's and `forensics/prerequisites.json` had assigned it here since before 6.10 began; the ledger's reading is the operative one and D128 records the correction. `RT-CONF-MOD` gains 86 observations for it -- 247 in total -- including the reader's two inner raise sites, which are reachable only by *draining* the error queue, because `module_run` raises its own record over the initialiser's. |
...
| 6.12 | **Third-party provider court** | nothing new — the crown-jewel test | 6.5–6.11 | `RT-PROVIDER-3P` | **COMPLETE** (D133). The probe compiles an `OSSL_provider_init` into its own binary, registers it with `OSSL_PROVIDER_add_builtin`, loads it with `OSSL_PROVIDER_load_ex` and reports what the core handed it — the core's *provider-facing* table, which `OSSL_provider_init` and nothing else can see. 40 observations, no residuals. **The stated exit criterion is corrected here rather than met as written.** It said the provider yields "matching init dispatch, core upcalls, parameter flow, algorithm enumeration, property selection, operation calls, teardown and failure behaviour". Algorithm enumeration, property selection and operation calls are an `EVP` fetch, which is Phase 7's and Phase 9's: a probe that needed them would be comparing a missing subsystem rather than the core, so it deliberately does not, and the exit criterion is the dispatch walk (entries, digest, first id), `CORE_GET_LIBCTX`, `CORE_GET_PARAMS` including the config-parameter merge and an absent key left alone, the `CRYPTO_*` trio, `CORE_THREAD_START`, the child-callback pair and `OSSL_LIB_CTX_new_child` from inside `init`, and the teardown freeing the child. **The court found five defects**, every one of them invisible to the other fifty-five courts for the same reason — nothing had ever walked that table or taken that role: `FUNC_CORE_OBJ_ADD_SIGID`/`CORE_OBJ_CREATE` were 121/122 where the header says **11/12**; the BIO and `CRYPTO_*` groups were published in numeric order rather than `core_dispatch_`'s; `PROVIDER_UP_REF`/`PROVIDER_FREE` (110/111) were still marked "absent, 6.8c" three commits after 6.8c closed, and their absence is what made `OSSL_LIB_CTX_new_child` fail outright; `ossl_provider_register_child_cb` answered a literal `1` where the authority answers the push's count; and `ossl_provider_add_to_store` never called `create_provider_children`, so no registered parent was ever told a provider had become active. `D-CHILD-REGISTER-PROPS-1`'s "no court has observed either half" is updated: the registration half is observed and passes on both sides, the property-string step still is not |
