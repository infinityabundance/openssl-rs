...
| 6.8b-slot | Filling slot 1, and the construction-order question it exposed | `crypto/context.c`'s `context_init` | 6.8a, 6.8b | `RT-LIBCTX` | **OPEN, and deliberately not done on a guess.** The plan says 6.8 fills slot 1, and `context_init` is where the authority does it. Reading that function to find the right position turned up something larger: the authority builds **eighteen** slot objects in a documented priority order — `evp_method_store`, `provider_conf`, `drbg`, `decoder_store`, `decoder_cache`, `encoder_store`, `store_loader_store` (all marked *P2, must be freed before the provider store*), then **`provider_store`** (*P1, needs to be freed before the child provider data*), then `property_string_data`, `namemap`, `property_defns`, `global_properties`, `bio_core`, `drbg_nonce`, `self_test_cb`, `indicator_cb`, `threads`, `child_provider`, then `property_parse_init`, then `comp_methods`. **The candidate's `context_init` builds `threads` first**, and `threads` is the authority's *seventeenth*. The construction order is not directly observable — a caller sees only the finished table, and this profile's build disables the allocation-failure injection that would make the cascade order visible (`no-allocfail-tests`) — but `context_deinit`'s order *is* observable in principle, because a slot object's destructor has side effects. So this is a real fidelity question that 6.8a did not create and that adding slot 1 at an arbitrary position would entrench. It is recorded here as its own item: **reconcile `context_init` and `context_deinit`'s order with the authority's, then fill slot 1 in its documented position.** The `P1`/`P2` comments are the authority's own statement of the teardown constraints and are what the reconciliation has to satisfy |
| 6.8c | init, activate, deactivate, and the operation tables | `provider_core.c` §§947–2,300: `provider_init` (the `DSO_load` branch and the builtin branch, and the module-filename derivation), `provider_activate`/`_deactivate`, `provider_flush_store_cache`, `provider_remove_store_methods`, `ossl_provider_activate`/`_deactivate`/`_activate_fallbacks`, `provider_activate_fallbacks`, `ossl_provider_doall_activated`, `ossl_provider_teardown`, `ossl_provider_default_props_update`, `ossl_provider_query_operation`/`_unquery_operation`, `_set_operation_bit`/`_test_operation_bit`, and the exports `OSSL_PROVIDER_load`/`load_ex`/`try_load`/`try_load_ex`/`unload`/`available`/`do_all`/`gettable_params`/`get_params`/`self_test`/`get_capabilities`/`query_operation`/`unquery_operation`/`get0_provider_ctx` | 6.8a, 6.8b, 6.9 (the dynamic branch *is* a `DSO_load`), 6.7b (default properties) | `RT-PROVIDER` | this is the part the third-party provider court at 6.12 ultimately judges. The fallback chain, the activation refcount, the operation bitset and the store flush are all observable through the exported surface, and the dynamic branch becomes observable here because 6.9 has landed |
