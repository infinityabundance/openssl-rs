...
authority build executes inside the isolated court container.
...
The court image is pinned by digest:

```
base: debian@sha256:88200866dfff7ea7f5cbcb6ec7c8a701889efe6fe859fe64d6990e4b07ea4171
```

Resource caps (OOM protection), applied at `docker run` time:
...
`docker/openssl-rs-court.sh verify` asserts the caps are in force and that no
non-authority `openssl` CLI is reachable.
