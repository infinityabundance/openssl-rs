...
## D202 — 0.0.11: the version moves, 62 declarations move with it, and the receipts stay where they are

Cutting `0.0.11` after the Phase-7 seal. `docs/RELEASE_GATES.md` §8's sequence was followed in
order, and this entry records the one relationship in it that a reader would otherwise have to
work out: **the version is an input to a generated artefact, so a bump that does not re-run the
generator is a silent lie, while a bump that re-runs it changes 62 declarations and no evidence.**

What moved, and by what:

| | before | after | how |
|---|---|---|---|
| `Cargo.toml` `[package] version` | 0.0.10 | **0.0.11** | edited — the one place |
| `Cargo.lock`'s `openssl-rs` entry | 0.0.10 | **0.0.11** | regenerated by the build; the lock records the crate's own version, which is how `cargo publish`'s dirty-tree refusal found it last time |
| the 62 FRF `manifest.yaml` `version_or_commit` fields | 0.0.10 | **0.0.11** | `python3 forensics/tools/gen_frf_courts.py`, which reads the version from `Cargo.toml` through `candidate_version()` rather than a second literal |

**What deliberately did not move, and why.** `grep -rl '0\.0\.10'` still finds it in
`docs/DECISIONS.md`, `docs/RELEASE_GATES.md` and `docs/PHASE-7-EVP-SEAL.md` (all history: the
first two are append-only and the seal's line 416 is a *report of what the compiled claim says*,
not a live assertion about the crate — the claim `96750dc6…` binds candidate
`openssl-rs 0.0.10 (e4f60d8b)` because that is the string at capture time, and a bump does not
change what it says), and in `.frf/receipts/*` and `.gemel/objects/*`, which are **evidence**.
Editing a receipt to make it name a newer version would destroy the thing that makes it
evidence; the receipts carry `identity_hash` `e4f60d8b…`, which is the *code* identity, and the
bump changed no code, so they remain valid for this distribution and FRF continues to accept the
store. That the store already held a claim naming `0.0.9` while `Cargo.toml` said `0.0.10` is
D200's finding, unchanged here.

A **gap is left at D201 on purpose**: the in-flight `phase8-digests` branch carries D201 (the
correctness-vector plane), and taking that number here would collide when it merges. Gaps in this
file are already precedented (D144 and D150 do not exist, as the Phase-7 seal records).
