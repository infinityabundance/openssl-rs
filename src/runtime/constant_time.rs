...
//! ct_expectations.txt`, from the tracked `court/gen-constant-time-values.c` — rather than by
//! properties this file asserts about itself.
//!
//! SPDX-License-Identifier: Apache-2.0

// Nothing in the crate calls these yet, and that is a state rather than an oversight: the caller is
// `ssl3_cbc_digest_record` (`ssl/record/methods/ssl3_cbc.c`), which is this stratum's next unit and
// which removes this attribute. Restricting the module to the seven helpers that unit uses is what
// keeps the allow narrow -- the header's `BN_ULONG` domain, its 32- and 64-bit masks and its `_int`
// wrappers are absent rather than transcribed-and-unused.
#![allow(dead_code)]
...
mod tests {
    ...
    /// The authority's own header output, generated by the tracked `court/gen-constant-time-values.c`
    /// and committed at `courts/phase8/ct_expectations.txt`.
    ///
