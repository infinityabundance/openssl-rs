...
/// The authority translation unit whose bodies reach `ossl_cipher_generic_reset_ctx`'s one
/// allocation, for the allocation-tracking `file` argument.
///
/// **Not the general cipher `file`, and deliberately without the source-tree prefix.**
/// `courts/layout/oracle-mem-file.c` installs `CRYPTO_set_mem_functions` and prints every `file`
/// the authority hands a caller-installed allocator: `providers/implementations/ciphers/ciphercommon.c`
/// is what `ciphercommon.c.in` compiles to, a `.c.in` template the build expands **into the build
/// tree**, so its `__FILE__` carries no `../../src/openssl-3.6.4/` prefix where every source-tree
/// sibling's does. The same program shows this TU is reached by exactly one of the crate's
/// allocation sites -- the `tlsmac` release below -- and by no *row's* `newctx`, which is why each
/// row now carries its own constant.
const FILE_CIPHERCOMMON: *const c_char =
    c"providers/implementations/ciphers/ciphercommon.c".as_ptr();

/// The authority translation units the per-row allocations belong to, measured by the same
/// program.
///
/// **A row's `newctx` is not in `ciphercommon.c`.** The generic rows' context constructors are
/// generated by the `IMPLEMENT_generic_cipher` macro, which
/// `providers/implementations/include/prov/ciphercommon.h:194-225` *defines* but
/// `cipher_aes.c`, `cipher_camellia.c`, `cipher_tdes_common.c`, `cipher_sm4.c`, `cipher_aria.c`
/// and `cipher_idea.c` each *invoke* -- so `__FILE__` inside an expanded body is the invoking
/// file, not the header's and not the header-only engine's. One constant naming `ciphercommon.c`
/// was therefore wrong for every one of the crate's former sites, and `file` is part of
/// `CRYPTO_set_mem_functions`'s published contract, so the string reaches an application.
///
/// The CTS rows are the exception that proves the rule about `.inc` files: their dispatch tables
/// are embedded into `cipher_aes.c` and `cipher_camellia.c` too (`cipher_cts.c`'s own comment says
/// so, and `cipher_cts.c`'s object contains no allocator `file` at all), so they share these two
/// constants rather than having a `FILE_CTS`.
const FILE_AES: *const c_char =
...
const FILE_AES_XTS: *const c_char =
    c"../../src/openssl-3.6.4/providers/implementations/ciphers/cipher_aes_xts.c".as_ptr();
const FILE_AES_OCB: *const c_char =
    c"../../src/openssl-3.6.4/providers/implementations/ciphers/cipher_aes_ocb.c".as_ptr();
...
unsafe extern "C" fn aes_freectx(vctx: *mut c_void) {
    ...
    unsafe {
        ...
        CRYPTO_clear_free(vctx, core::mem::size_of::<ProvAesCtx>(), FILE_AES, LINE);
    }
}
...
unsafe extern "C" fn aes_dupctx(ctx: *mut c_void) -> *mut c_void {
    ...
    unsafe {
        ...
        let ret = CRYPTO_malloc(core::mem::size_of::<ProvAesCtx>(), FILE_AES, LINE);
        if ret.is_null() {
            return ptr::null_mut();
        }
...
unsafe extern "C" fn camellia_freectx(vctx: *mut c_void) {
    ...
    unsafe {
        ...
        CRYPTO_clear_free(
            vctx,
            core::mem::size_of::<ProvCamelliaCtx>(),
            FILE_CAMELLIA,
            LINE,
        );
    }
...
unsafe extern "C" fn camellia_dupctx(ctx: *mut c_void) -> *mut c_void {
    ...
    unsafe {
        ...
        let ret = CRYPTO_malloc(core::mem::size_of::<ProvCamelliaCtx>(), FILE_CAMELLIA, LINE);
        if ret.is_null() {
            return ptr::null_mut();
        }
...
unsafe extern "C" fn tdes_freectx(vctx: *mut c_void) {
    ...
    unsafe {
        ossl_cipher_generic_reset_ctx(vctx.cast());
        CRYPTO_clear_free(vctx, core::mem::size_of::<ProvTdesCtx>(), FILE_TDES, LINE);
    }
}
...
unsafe extern "C" fn tdes_dupctx(ctx: *mut c_void) -> *mut c_void {
    ...
    unsafe {
        ...
        let ret = CRYPTO_malloc(core::mem::size_of::<ProvTdesCtx>(), FILE_TDES, LINE);
        if ret.is_null() {
            return ptr::null_mut();
        }
...
macro_rules! cipher_row {
    ($newctx:ident, $getparams:ident, $table:ident, $ctx:ty, $hw:path, $kbits:expr,
     ...
     $gettable:path, $settable:path, $file:expr) => {
...
unsafe fn aes_wrap_newctx(
    ...
    unsafe {
        ...
        let wctx = CRYPTO_zalloc(core::mem::size_of::<ProvAesWrapCtx>(), FILE_AES_WRAP, LINE);
        if !wctx.is_null() {
            ossl_cipher_generic_initkey(
                wctx,
                kbits,
...
unsafe extern "C" fn aes_wrap_dupctx(wctx: *mut c_void) -> *mut c_void {
    ...
    unsafe {
        ...
        let dctx = CRYPTO_memdup(
            wctx,
            core::mem::size_of::<ProvAesWrapCtx>(),
            FILE_AES_WRAP,
            LINE,
        );
        if !dctx.is_null() {
            ...
            if !(*d).base.tlsmac.is_null() && (*d).base.alloced != 0 {
                ...
                if tm.is_null() {
                    CRYPTO_free(dctx, FILE_AES_WRAP, LINE);
                    return ptr::null_mut();
                }
...
macro_rules! cts_row {
    ($newctx:ident, $getparams:ident, $table:ident, $ctx:ty, $hw:path, $kbits:expr,
     $freectx:path, $dupctx:path, $file:expr) => {
        unsafe extern "C" fn $newctx(provctx: *mut c_void) -> *mut c_void {
            if is_running() == 0 {
                return ptr::null_mut();
            }
...
unsafe fn aes_xts_newctx(
    ...
    unsafe {
        ...
        let ctx = CRYPTO_zalloc(core::mem::size_of::<ProvAesXtsCtx>(), FILE_AES_XTS, LINE);
        if !ctx.is_null() {
            ossl_cipher_generic_initkey(
                ctx,
                kbits,
...
unsafe extern "C" fn aes_xts_freectx(vctx: *mut c_void) {
    ...
    unsafe {
        ...
        CRYPTO_clear_free(
            vctx,
            core::mem::size_of::<ProvAesXtsCtx>(),
            FILE_AES_XTS,
            LINE,
        );
    }
...
unsafe extern "C" fn aes_xts_dupctx(vctx: *mut c_void) -> *mut c_void {
    ...
    unsafe {
        ...
        let ret = CRYPTO_malloc(core::mem::size_of::<ProvAesXtsCtx>(), FILE_AES_XTS, LINE);
        if ret.is_null() {
            return ptr::null_mut();
        }
        let hw = (*in_).base.hw;
...
unsafe fn aes_ocb_newctx(
    ...
    unsafe {
        ...
        let ctx = CRYPTO_zalloc(core::mem::size_of::<ProvAesOcbCtx>(), FILE_AES_OCB, LINE);
        if !ctx.is_null() {
            ossl_cipher_generic_initkey(
                ctx,
                kbits,
...
unsafe extern "C" fn aes_ocb_freectx(vctx: *mut c_void) {
    ...
    unsafe {
        if !vctx.is_null() {
            ...
            CRYPTO_clear_free(
                vctx,
                core::mem::size_of::<ProvAesOcbCtx>(),
                FILE_AES_OCB,
                LINE,
            );
        }
...
unsafe extern "C" fn null_newctx(_provctx: *mut c_void) -> *mut c_void {
    ...
    CRYPTO_zalloc(core::mem::size_of::<ProvNullCtx>(), FILE_NULL, LINE)
}
...
unsafe extern "C" fn null_freectx(vctx: *mut c_void) {
    ...
    unsafe { CRYPTO_clear_free(vctx, core::mem::size_of::<ProvNullCtx>(), FILE_NULL, LINE) };
}
