...
// Phase 8.7's forward landing of the two BN units `crypto/ec/`'s closure reaches (D338):
// `exp` is `crypto/bn/bn_exp.c`'s one internal, and `intern` is `crypto/bn/bn_intern.c`'s
// eight. Both land before their callers, which is the plan's order rather than an accident;
// neither defines an export, so neither moves a ledger row.
pub(crate) mod exp;
pub mod gf2m;
